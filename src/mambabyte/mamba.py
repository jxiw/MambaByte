...
class Mamba(nn.Module):
    def __init__(
        ...
        disable_optimizations=False,  # for visualization
        layer_idx=None,
        device=None,
        dtype=None,
    ):
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.disable_optimizations:
            xz = self.in_proj(hidden_states)  # (b, L, d) -> (b, L, 2 * d_inner)
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            ...
            if self.disable_optimizations:
                dt = rearrange(dt, "(b l) r -> b l r", l=seqlen)  # r: dt_rank
            ...
            y = selective_scan_fn(
                ...
                self.D.float(),
                z=z,
                delta_bias=self.dt_proj.bias.float() if not self.disable_optimizations else None,
                delta_softplus=True,
    ...
    def _get_states_from_cache(self, inference_params, batch_size, initialize_states=False):
        ...
        if self.layer_idx not in inference_params.key_value_memory_dict:
            ...
            ssm_state = torch.zeros(
                ...
                dtype=self.dt_proj.weight.dtype,  # dtype=torch.float32,
            )
