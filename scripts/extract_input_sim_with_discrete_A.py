...
from src.utils.visualization_utils import get_input_sim_with_discrete_A
...
def main(
    ...
    prediction_artefacts = get_input_sim_with_discrete_A(
        model=model, layer_idxs=layer_idxs, prompt=prompt, return_log_probs=return_log_probs, return_ranks=return_ranks
    )

    return_dict = {
        "prompt": prompt,
        "input_sim_with_discrete_A": model.hooks["outputs"]["input_sim_with_discrete_A"],
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Extract input similarity with discrete-A matrix at each timestep.")
